...
class QuantizeLinear(nn.Linear):
    ...
    def forward(self, input_, a_bit_run, w_bit_run):
        # quantize weight
        ...
        if w_bit_run >= 32:
            ...
            weight = SymQuantizer.apply(
                real_weights, weight_clip_val, w_bit_run, self.weight_layerwise
            )
        else:
            if w_bit_run == 1:
        ...
        # Quantize inputs
        if a_bit_run < 32 and a_bit_run > 2:
            act_clip_val = torch.tensor([-2.0, 2.0])
            input_ = self.act_quantizer.apply(
                input_, act_clip_val, a_bit_run, self.act_layerwise
            )

...
def main():
    ...
    if (not args.only_eval):
        dic = torch.load('pytorch_model_t.bin',map_location='cpu')                 
        new_dic = {}
        for k in dic.keys():
    ...
    def get_quant_bit(iter, max_steps, cycle = 32):
        B_min, B_max = 0,3
        T_n = max_steps/cycle
        B_t = B_min + 0.5 * (B_max - B_min) * (1 - np.cos(2 * np.pi * iter / T_n))
        
        if B_t < 0.7499999999999998:
            return 4,4,2
        if B_t < 2.2499999999999996:
            return 8,8,1
        return 32,32,0
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        if not args.only_eval:
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                w_bit, a_bit, bit_index = get_quant_bit(total_steps, args.max_train_steps)
                ...
                run_loss_main = run_loss_main*0.8 + 0.2*loss.item()
        ...
        if (not args.only_eval):
            torch.save(model.state_dict(),str(args.p_name) + "_" + str(epoch) + "_pytorch_model.bin")
        if (args.only_eval):
            assert 5==6+1, "FALSE!"
...
# t -> goes from 0 to t/4 & in smaller up case 0 to t/8
# 2 * 180 * 4 * t 
# 360 * 4 * t -> t = t/8 -> -1
# x*t/8  * 360 * 4 -> x*180 -> 60/60/60